...
{
	var hours = new string[] { "12a", "1a", "2a", "3a", "4a", "5a", "6a", "7a", "8a", "9a", "10a", "11a", "12p", "1p", "2p", "3p", "4p", "5p", "6p", "7p", "8p", "9p", "10p", "11p" };
	var days = new string[] { "Saturday", "Friday", "Thursday", "Wednesday", "Tuesday", "Monday", "Sunday" };

	var dataFunc = new JavascriptFunction(@"
function getData() {
...
");
	...
	var options = new ChartOptions()
	{
		...
		Legend = new()
		{
			Data = new string[] { "Punch Card" },
			Left = "right"
		},
		Tooltip = new()
		{
			Position = TooltipPositionType.Top,
			Formatter = new JavascriptFunction("function (params) { return ( params.value[2] + ' commits in ' + hours[params.value[0]] + ' of ' + days[params.value[1]] ); }")
		},
		Grid = new()
		{
			Left = 2,
...
Console.WriteLine($"allocated after = {GC.GetTotalAllocatedBytes(precise: true)}");

GC.Collect();
Console.WriteLine($"allocated after GC = {GC.GetTotalAllocatedBytes(precise: true)}");
