...
public partial class Dataset
{
	...
	/// <summary>
	/// Source data.
	/// Generally speaking, a source data describe a table, where these format below can be applied:  
	...
	/// Optional values:   null/undefined/'auto' : means auto detect whether the first row/column is dimension names or data.
	///  true : the first row/column is dimension names.
	///  false : data start from the first row/column.
	///  number : means the row/column count of the dimension names, that is, the start index of data row/column.
	/// e.g.
	/// sourceHeader: 2 means the front two rows/columns are dimension names, the back ones are data.
	///   
